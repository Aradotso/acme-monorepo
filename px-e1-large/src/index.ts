...
/** Remove HTML-like tags without interpreting entities; malformed/unclosed tags are removed to the end. */
...
/** Center text in a field using the requested fill character; invalid width/fill returns the original text. */
export function padCenter(input: string, width: number, fill = ' '): string {
  const length = [...input].length;
  if (width <= length || !fill) return input;
  const total = width - length;
  const left = Math.floor(total / 2);
  return fill.repeat(Math.ceil(left / [...fill].length)).slice(0, left) + input + fill.repeat(Math.ceil((total - left) / [...fill].length)).slice(0, total - left);
}
...
export function levenshtein(a: string, b: string): number {
  ...
  return previous[right.length - 1] ?? left.length;
}
